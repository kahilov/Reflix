...
class App extends Component {
  ...
  handleRent = (e, id) => {
    let movieArr = [...this.state.catalog];
    let movieIndex = movieArr.findIndex(m => m.id === id);
    movieArr[movieIndex].isRented = !movieArr[movieIndex].isRented;
    this.setState({ catalog: movieArr });
  }
  render() {
      ...
      <Router>
        <div className="App">
          ...
          <Route
            path="/catalog"
            exact
            render={() => <Catalog changeSearch = {this.changeSearch} handleRent ={this.handleRent}state={state} />}
          />
          <Route
            path="/catalog/movie/:movieDetail"
            exact
